...
export class FoxWalletAdapter extends BaseMessageSignerWalletAdapter {
  ...
  get url() {
    if (this._isMobile) {
      ...
      const url = cbUUID
        ? `https://${location.host}/quest/coinbase?uuid=${cbUUID}&next=${location.href}`
        : location.href;
